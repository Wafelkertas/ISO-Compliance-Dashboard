...
function getColorClasses(level: "green" | "yellow" | "red" | "gray") {
    switch (level) {
        ...
        case "red":
            return {
                text: "text-red-700", bg: "bg-red-50", border: "border-red-200", bar: "bg-red-400",
            };
        default:
            return {
                text: "text-gray-700", bg: "bg-gray-50", border: "border-gray-200", bar: "bg-gray-400",
            };
    }
}
...
function RiskCard({item, total}: { item: RiskItem; total: number }) {
    ...
    return (<div className={`p-4 rounded-xl border ${colors.border} ${colors.bg}`}>
...
export function RiskMetrics() {
    ...
    return (<div className="bg-white rounded-lg border border-gray-200 p-6">
            <div className="mb-6">
                <p className="text-gray-600">Total Resiko Teridentifikasi</p>

                {loading ? (<p className="text-gray-900 mt-1">Loading…</p>) : error ? (
            ...
            <div className="space-y-4">
                ...
                {risks.map((risk) => (<RiskCard key={risk.level} item={risk} total={totalRisks}/>))}
            </div>

            <div className="mt-6 pt-6 border-t border-gray-200">
                <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600">Tingkat Mitigasi Risiko</span>
